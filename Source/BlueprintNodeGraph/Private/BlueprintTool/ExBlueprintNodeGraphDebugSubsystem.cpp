...
static bool IsProxyFinished(UObject* ProxyObject)
{
	...
	if (const UExAsyncActionBase* AsyncProxy = Cast<UExAsyncActionBase>(ProxyObject))
	{
		return AsyncProxy->IsFinished();
	}
	if (const UExLatentActionProxyBase* LatentProxy = Cast<UExLatentActionProxyBase>(ProxyObject))
	{
